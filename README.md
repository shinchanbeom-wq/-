...
### CMD에서 한글 깨짐/이상한 명령 오류가 날 때
배치 파일 인코딩 문제로 명령이 깨질 수 있어, 모든 `.bat` 스크립트를 영문 출력만 사용하도록 수정했습니다.
그래도 같은 문제가 있으면 아래 순서로 실행하세요.
1. `cmd`를 새로 열고 프로젝트 폴더로 이동
2. `chcp 65001` 실행
3. `one_touch_setup.bat` 실행
